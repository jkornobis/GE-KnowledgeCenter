...
- **Software Engineer — `git stash -q` … `git stash pop -q` is how a before/after claim is made over a checker.** Used today to establish that a change introduced no new conformance warnings rather than asserting it — a reading of the branch at that moment, `8 before, 8 after`, and worth carrying with the technique because the finished branch cleared two and landed `8 → 6`. **A before/after pair is true of the state it was taken from, not of the change it ends up describing.** **A stash with nothing to stash silently succeeds and the following `pop` fails**, which reads as an error and is not one.
