...
description: "The component workbench where a variant becomes a runnable story: 1,098 of 1,571 open issues carrying the bug label, three sampled and all three marked has-workaround, and friction that is all build pipeline rather than authoring"
status: draft
...
Audited 2026-09-24 by User Researcher (re-audit; first pass 2026-08-23). Library (dev server + build tool), not MCP-mediated. No auth; runs against a project's own build pipeline.

Re-audit: 2 days — measured, release cadence (n=8 intervals, agile-watcher @ acaadb3, 2026-08-30)

**Re-audited 2026-09-24 ahead of the clock**, under the upkeep ratio (`protocols/library-conventions.md`). As the most overdue of seven tied pages, none of which any instance had read in 19 days, it went first. **What moved since 2026-08-23:** the bug count (1,102 → 1,098), Context7's numbers (below), and the release state. **The latest stable is `v10.6.0` (2026-09-02), with none since, while `v11.0.0-alpha.1` shipped 2026-09-19.** Stable releases came 2–7 days apart in July and August, then 13 days, then 22 days and counting. **The next event that can invalidate this page is v11, not the calendar.** Every issue, question and Critical fact below was re-read live on 2026-09-24.
...
| Field | Value |
|---|---|
| Library ID | `/storybookjs/storybook` — resolved 2026-08-23, re-resolved 2026-09-24, **not recalled** |
| Snippets · trust · benchmark | 6,862 · 8.8 · **84.18** (2026-09-24) — was 8,423 · High · 80.56 on 2026-08-23 |
...
Versions are pinnable here (`v10.2.9`, `v9.0.15`, `v8_6_14`, `v6_5_9`, the same four on 2026-09-24, so the current `v10.6` is **not** a pinnable id), which matters more than usual — see the defects below, where the standing bugs are version- and framework-specific.
...
- **QA Engineer** — `@storybook/addon-a11y` runs axe-core against every story (re-verified 2026-09-24: `10.6.0` on npm depends on `axe-core ^4.2.0`), which shifts WCAG left into the component library rather than an end-of-line audit. Everything on `axe-core.md` — the ~30–40% ceiling, the false-positive base rate, the composited-colour trap — applies here unchanged, because it is the same engine.
...
### Defects — the project's tracker, read 2026-08-23 and re-read 2026-09-24 via the GitHub API

**1,571 open issues, of which 1,098 carry the `bug` label** (2026-09-24; 1,102 on 2026-08-23) — and unlike Playwright, this project *does* label bugs, so the query means what it says. That is a real standing defect load, and it is the honest counterweight to a tool this widely adopted.
...
### Friction — Stack Overflow, read through the Browser pane 2026-08-23, re-read through its API 2026-09-24

**2,133 questions**, the same count a month later with the same top five, so the tag is quiet. Their shape is different from Playwright's in a way that matters to whoever adopts it.
