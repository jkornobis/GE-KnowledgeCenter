...
- **Two ways to map, and a third arrived at the MCP layer 2026-09-22** ([Figma docs](https://developers.figma.com/docs/code-connect/)): the **CLI** with framework-agnostic TypeScript template files (recommended, works with any language/framework), the **UI** with GitHub integration and one-to-many mapping (one design component → its React / Swift / Android implementations), and now **`get_code_connect_suggestions` → review with the Composer → `send_code_connect_mappings`** — AI-suggested node-to-component matches, confirmed rather than authored, then bulk-saved. Map the design system's real components, not one-offs, whichever route writes the mapping. Schema-verified only — `tools/figma-mcp-remote.md`'s re-audit found the tools present and has not run either against a real file yet.
...
Config 2026 shipped surfaces next to this canvas method ([Config 2026 recap](https://www.figma.com/blog/config-2026-recap/)). Not part of the perceive-loop, but the honest choice when the task calls for them — and as of the MCP connector's 2026-09-22 re-audit (`tools/figma-mcp-remote.md`), two of these stopped being UI-only and became directly callable:

- **Shader fills/effects** — **MCP-reachable now**: `list_shaders`, `get_shader`, `create_shader`, `update_shader`, `list_file_shaders`. Read-only became full CRUD in the 2026-08-25→09-22 interval. `create_shader`/`update_shader` both require the `figma-shaders` skill loaded first — same *"causes common, hard-to-debug failures if skipped"* shape as `figma-use`.
- **Generative Plugins** — **MCP-reachable now**: `list_generative_plugins`, `get_generative_plugin`, `create_generative_plugin`, `update_generative_plugin`. This family did not exist on the connector at all as of 2026-08-25. Requires the `figma-generative-plugins` skill first, same pattern.
- **Figma Motion** — **partially MCP-reachable**: `get_motion_context` (read keyframe data) and `export_video` (render to MP4) exist; authoring a timeline is still canvas-only. `method/figma-motion.md` carries the keyframe/timing fundamentals recorded from a reference document, not yet verified against this live API.
- **Code Layers** and the **enhanced Figma Agent** (custom skills + connectors) — **still not MCP-reachable**, checked live 2026-09-22: no `code_layer`-shaped tool exists on the connector, and the Agent's connectors are a product-UI feature, not a tool a session calls. These remain the two genuinely UI-only items on this list.

Steer with the feature's real name — and for the first three, that now means the actual tool name, not just naming the surface and reaching for `use_figma` to hand-build it.
