...
**A fourth delegate exists and is not an MCP connection.** Where a tool hosts its own AI agent — Figma's, for one — that agent is reachable by driving a browser to its chat panel, and it is **neither** of the two mechanisms this page is about: no protocol, no schema, no tool list, and not spawnable from this harness. **Its tokens are the vendor's.** The shape, its four constraints and the hard line that verification does not delegate are in `method/evidence.md`, beside the persona-versus-subagent economics, because that is where a musician already goes to choose between delegates. Named here so a reader arriving at *MCP or subagent* knows there is a third answer.
