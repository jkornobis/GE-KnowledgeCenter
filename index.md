...
it has checked and passed. **And three bare names are exceptions to the history rule, because they are runnable
files in this bundle rather than another estate's coordinates: `check_okf.mjs`, `check_links.mjs` and
`sweep_links.mjs`** — the first two are gates and fail a merge; the third is a sweep, reports and
never blocks.
