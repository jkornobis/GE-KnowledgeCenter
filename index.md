...
**Three fail a merge:** `check_okf.mjs` reads conformance and, since 2026-09-23, coverage (the index
against the disk, and new rows against the routing card in `start.md`), `check_links.mjs` reads resolution,
...
⚠️ **`Published` in the table below is the last edit, never the audit** (`#145`). The audit date is
the page's own `Audited` line, and the two drift apart: on 2026-09-23 a reader took
`figma-mcp-desktop`'s 2026-09-22 for a re-audit, and the page still says `Audited 2026-07-04`.
**Read the page's own line before calling a tool page fresh.**
...
| `tools/lineage.md` | Twelve chairs, each with the origin it started from, the leap that changed it, the current best-in-class, and the years between the two — the index a chair's tool pages hang from | the whole room | 2026-09-24 |
...
| `tools/storybook.md` | The component workbench where a variant becomes a runnable story: 1,098 of 1,571 open issues carrying the bug label, three sampled and all three marked has-workaround, and friction that is all build pipeline rather than authoring | Design Engineer, Software Engineer, QA Engineer | 2026-09-24 |
...
| `tools/README.md` | What a tool page records and why: the vantage point a reading was taken from, its verification state, and the three layers that stop a toolset being hand-rolled | the whole room | 2026-09-24 |
...
| `tools/register-sweeps.md` | The dated passes over the lever register: sweep 1 of 2026-08-05 and its scored candidates, sweep 2 of 2026-09-04 and what it was obliged to do — kept as the record of how the register came to say what it says, and correct only as of those days | the whole room | 2026-09-24 |
...
| `tools/figma-method.md` | Eyes first and code second: the working loop for a design surface, what the eyes read in a structured file, and why text-only spatial reasoning collapses as a layout grows | the whole room | 2026-09-24 |
...
| `protocols/triggers.md` | Full semantics and response formats for every typed trigger, the rule that a mode persists only by being echoed, and why breadth is a single-shot Auditorium rather than a sustained mode | jkornobis | 2026-09-24 |
...
| `method/figma-motion.md` | The perception-first rule for motion design (the eye sees pixels, never layers or z-order), keyframe/interpolation/timing basics, the twelve animation principles trimmed to what a UI needs, and what is specific to Figma Motion's own timeline and component/instance animation scope | UX Designer, Design Engineer | 2026-09-21 |
| `method/figma-team-map.md` | Enumerating a whole Figma team is not reachable through any of the 37 MCP tools this library has audited — it is a REST API tree-walk, over an endpoint family that changed shape recently enough to break a procedure that worked a month ago | UX Designer, Design Engineer, User Researcher | 2026-09-22 |
| `method/reading-acting-surface.md` | Cross-industry research (OpenAI Codex, Google Antigravity, Cursor, Cognition Devin) distilled into vendor-agnostic principles for how an agent surface should show progress and take a correction: evidence over logs, soft-steer distinct from hard-interrupt, and a fleet view GE only partly has | UX Designer, Product Owner, Design Engineer, the whole room | 2026-09-23 |
| `method/agent-kanban-git-board.md` | Four findings from a live corner of the ecosystem (GitHub's own Issues/Projects agent integration, Hermes Kanban's multi-agent dispatcher, Shipward's fix for the self-report problem, and the dedicated Scrum Master agent persona now packaged as its own Claude Code Skill): a third corroboration of this skill's oracle rule, four mechanisms this estate's Wekan+Forgejo setup does not have, and independent validation that the Agile Facilitator's separation from the working chairs is a real, converged-on pattern | Product Owner, Software Architect, Agile Auditor, the whole room | 2026-09-23 |
| `method/agent-surface-invariants.md` | A distillation of two days' research (OpenAI Codex, Google Antigravity, Cursor, Devin, GitHub Projects, Hermes Kanban, Shipward, the Scrum Master agent pattern) down to what actually recurs once the vendor and the surface are stripped away — and where each already exists, or doesn't, in this skill's own design | Product Owner, UX Designer, Software Architect, the whole room | 2026-09-23 |
| `method/agent-surface-time-axis.md` | Axis D from method/agent-surface-invariants.md (abandoned work surfaces on a clock), traced against this estate's own history: method/time-management.md's Ambient/Metronome pair (2026-08-28) was a human-attention tool, built before Forgejo/Wekan existed to give a machine-facing time axis anything to watch — three concrete forms the real gap could take now, named as options, not built | Product Owner, Software Architect, Agile Auditor, the whole room | 2026-09-23 |
| `method/agent-chair-industry-map.md` | Extends the Scrum Master confirmation (agent-kanban-git-board.md) across all twelve chairs: a per-chair search for a dedicated, market-named AI agent category doing that same job — seven confirmed as converged-on industry categories, two fragmented/partial, and Software Architect found as the symmetric reverse case: a chair GE names that the industry has not | Product Owner, Software Architect, Agile Auditor, the whole room | 2026-09-23 |
...
| `method/the-fourth-verdict.md` | Why every binary classifier that can fail to look must carry a state saying so, distinct from "there is nothing there": the asymmetry that the extra verdict describes the instrument rather than the world, why the collapse always biases toward false absence, the near-collision between two blank-looking states with opposite meanings and the guard it earns, and four dated implementations with what the missing verdict cost each | Agile Auditor, QA Engineer, Software Architect | 2026-09-15 |
| `method/conway-on-a-corpus.md` | Why a record built by asking each part "which of this is yours" undercounts reach by construction, invisible from inside any one part, fixed not by asking harder but by reading whatever surface in the same system was written between the parts rather than inside them — the dated case that found it: one chair recorded, a shared-concern table naming seven | Agile Facilitator, Software Architect, Content Designer | 2026-09-15 |
...
| **Does a router page owe an `Audited` date?** `tools/figma.md` carries a `Re-audit:` line since 2026-09-02 and no `Audited` line, and argues it cannot honestly hold one, since it carries no findings of its own. *The rest of this row was decided 2026-09-24 (`#145` item 9): `tools/README.md` no longer claims a checker runs here.* | whether a router page is exempt from the freshness rule, and whether that exemption is declared on the page or in the checker |
...
| `check_links.mjs` cannot tell a **route** from a **provenance marker** — when first counted, 111 bare names existed nowhere here: 108 provenance, and 3 links a reader would follow. **Re-counted 2026-09-24: 132** (119 on main, plus 13 once `#147` made the check read capitals). The 108/3 split was not re-derived | whether a markdown-linked bare name should fail the check while a cited one does not |
