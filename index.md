...
it has checked and passed. **And six bare names are exceptions to the history rule, because they are runnable
files in this bundle rather than another estate's coordinates: `check_okf.mjs`, `check_links.mjs`,
`sweep_links.mjs`, `propose_vectors.mjs`, `measure_stability.mjs` and `find_perturbations.mjs`** — the first two are gates and
...
whether this corpus carries the stability signature of a natural network, and one ranking the
bodies that sit furthest from what the structure predicts.
