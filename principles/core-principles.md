...
### Two cases the rule does not reach — proposed by the Workshop, 2026-09-04

**Occasion.** The Workshop instance recorded **seven** fresh misses of this pattern in a single session, against a row this estate marks **Fired**, and a page whose own heading records the pattern as invisible for six occurrences. **Three were this rule exactly**, and needed nothing new: a browser pane reporting an empty tree and `innerHeight: 0` when its viewport was 0×0 and settable — one `resize_window` fixed it; a `grep` on `.git/hooks/pre-push` reporting the push gate empty when `core.hooksPath` was `scripts/hooks`; a `grep "project/"` concluding a checker was independent of a directory it loads at line 37 via `read("project", "graph", …)`.

**The other four are outside the rule as written, and the asymmetry paragraph above is where the first gap opens.**

**1 — A quantity is neither a capability claim nor a limit claim, and this is that paragraph's own third case rather than a rule beside it.** The division above is not exhaustive: capability claims need an artifact, limit claims get accepted on silence. **A count is neither**, so it passes both gates and arrives looking like a measurement. Three of the seven were counts: a `grep -o` that summed each record's usage object twice (468 occurrences against 235 records, inflating a spend table roughly 2× and misreporting its shares as 41/37 when they were 49/25); a receipt tally that counted one 37-agent event as three, because **a session investigating an event quotes that event's text into its own transcript**, making the investigator its own second culprit; and timestamps read in UTC by an estate that renders Paris, placing an 11:10 CEST event at 10:56 in a report and two hours early on a chart.

**The proposed reading, offered as an extension rather than a new law:** *a quantity is a limit claim about every case the instrument did not count.* A pattern that misses is silent about what it missed, exactly as a pane returning nothing is silent about what it could not see. If that holds, this is the same move the rule already made once — invariant 2 reaching one layer further, from the model's claims to an instrument's readings, and now to an instrument's arithmetic.

**And the remedy is already named on this page.** In the same session, one measurement was correct first time: asking whether a repository existed, a bare 404 was ambiguous, so a **known-absent case went through the same instrument** — the real repository returned exit 0 with no branches, the invented one exit 128 with an explicit error. That is a **pseudo-oracle**, cited below, applied to a count instead of a capability. It cost one extra command.

**2 — A document is not an instrument, and nothing here reaches a sentence.** All six original occurrences are tools: a shell, WSL probes, `read_page` inside an iframe, `document.hasFocus()`, an MCP's own inventory. The seventh miss was a `.gitignore` comment describing its own repository as *"the public github.com remote"* — twice. **The repository was private, and had been since the day it was created.** A session read the comment as evidence rather than querying the host, and the false premise produced an exposure argument, two decisions put to a Composer on a risk that did not exist, and incorrect text in three published artifacts.

**Why this one is worse than a broken tool.** A tool fails loudly and often, so its output is already suspect. **A stale comment returns no error, carries the authority of the document it lives in, and sounds specific.** It was not wrong about its rule — [EMPLOYER]-classified assets still do not belong in that tree — only about the world.

**The proposed rule, scoped deliberately narrowly:** *a document may not be the source of truth about a system's live configuration.* Hosts, visibility, permissions, remotes, versions — confirmed against the system that owns them, or not asserted. **This is not "verify all documentation"**, which is unaffordable and is not what failed; it is one class of claim, the class whose truth changes without the document changing.

**What the Workshop is not claiming.** Whether these belong inside this rule, beside it as a sibling, or in `method/evidence.md` is the Library's to decide — the Workshop found them and does not own where they live. **Both are stated as proposals and neither is written into the rule's own statement above.**
