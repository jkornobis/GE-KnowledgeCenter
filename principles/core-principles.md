...
**Why this one is worse than a broken tool.** A tool fails loudly and often, so its output is already suspect. **A stale comment returns no error, carries the authority of the document it lives in, and sounds specific.** It was not wrong about its rule — the ignored assets genuinely do not belong in that tree — only about the world.
